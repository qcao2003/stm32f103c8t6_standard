...
void OV7670_GPIO_Init(void)
{
	...
	//GPIOA13,14初始化设置
	GPIO_InitStructure.GPIO_Pin = GPIO_Pin_13|GPIO_Pin_14;//PA13,14推挽输出
	...
	RCC_APB2PeriphClockCmd(RCC_AHB1Periph_GPIOB, ENABLE);        /* GPIOB使能 */

	GPIO_InitStruct.GPIO_Pin=GPIO_Pin_0 |GPIO_Pin_10 | GPIO_Pin_11 | GPIO_Pin_12;  /* PB0，PB10,PB11,PB12 */
	GPIO_InitStruct.GPIO_Mode=GPIO_Mode_OUT;                   /* 推挽输出 */
	GPIO_InitStruct.GPIO_OType = GPIO_OType_PP;
	GPIO_InitStruct.GPIO_PuPd = GPIO_PuPd_UP;
	GPIO_InitStruct.GPIO_Speed=GPIO_Speed_50MHz;                   /*  50MHz*/
	GPIO_Init(GPIOB,&GPIO_InitStruct);
	GPIO_SetBits(GPIOB,GPIO_Pin_0 | GPIO_Pin_10 | GPIO_Pin_12);/* 给WRST  OE 和 RRST初始赋值 */
	GPIO_ResetBits(GPIOB,GPIO_Pin_11);/* 给 WEN 赋初值 */
	/* RCLK IO口初始化 */
	RCC_APB2PeriphClockCmd(RCC_AHB1Periph_GPIOB, ENABLE);        /* GPIOB使能 */
	...
	GPIO_SetBits(GPIOB,GPIO_Pin_13);/* 给RCLK初始赋值 */
	/* VS IO口初始化 */
	RCC_APB2PeriphClockCmd(RCC_AHB1Periph_GPIOB, ENABLE);        /* GPIOB使能 */
	...
	GPIO_Init(GPIOB,&GPIO_InitStruct); 
	/* D0-D7 IO口初始化 */
	RCC_APB2PeriphClockCmd(RCC_AHB1Periph_GPIOA, ENABLE);        /* GPIOA使能 */
...
void OV7670_STM32GetPic(void)
{
	...
	if(OV7670_State==2)
	{
		...
		for(i = 0; i < 128; i++) 
		{
			...
			{     
				OV7670_W_RCLK(0);
				//half_pixel = GPIOA->IDR&0xFF;			    //获得低八位
				//LCD_WR_DATA8(GPIOA->IDR&0xFF);
				*ptr = GPIOA->IDR&0xFF;
				...
				OV7670_W_RCLK(0);
				//half_pixel = GPIOA->IDR&0xFF;					//获得高八位
				//LCD_WR_DATA8(GPIOA->IDR&0xFF);
				*ptr = GPIOA->IDR&0xFF;
