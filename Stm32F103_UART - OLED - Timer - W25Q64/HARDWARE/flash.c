...
void SPI_Flash_Init(void)
{
  ...
  RCC_APB2PeriphClockCmd(	RCC_APB2Periph_GPIOA, ENABLE );
	...
	GPIO_InitStructure.GPIO_Pin = GPIO_Pin_2|GPIO_Pin_3|GPIO_Pin_4;  //SPI CS 片选信号
	...
	SPI1_SetSpeed(SPI_BaudRatePrescaler_16);	//设置为4M时钟，低速模式
...
//读取芯片JedecDeviceID W25Q64的ID:0XEF16
...
//WriteAddr:开始写入的地址(24bit)
//NumByteToWrite:要写入的字节数(最大65535)  		   
u8 SPI_FLASH_BUF[4096];  //一个扇区有4096个字节
...
{ 
	...
	secpos=WriteAddr/4096;//扇区地址 0~511 for w25x16 这个当前地址除以4096得到的是当前地址所在的第几扇区
...
void Write_page_with_check_Sector(u8* TEXT_Buffer,u32 FLASH_SIZE,u16 NumByteToWrite)
{
	...
	if(datatemp[0]!=0XFF)
		SPI_Flash_Erase_Sector((FLASH_SIZE)/4096);  //删除当前的扇区
