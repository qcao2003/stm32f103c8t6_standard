...
void OV7670_STM32GetPictoSDCARD(uint32_t weight,uint32_t height,FIL *fnew,FRESULT res,UINT fnum,const TCHAR* path)
{
	uint32_t i,j;
	u8 color;
	BITMAPINFO bmp;
	res = f_open(fnew, path, FA_OPEN_ALWAYS | FA_WRITE | FA_READ);
	if(res == FR_OK)
	{
		printf("Beignning writing\r\n");
		//填写文件信息头信息  
		...
		if(OV7670_State==2)
		{
			...
			printf("OV7670getPIC is completed\r\n");
			}
	}
}
...
//	u16 color;
	if(OV7670_State==2)
	{
		...
		for(i = 0; i < height; i++) 
		{
			...
			{     
				OV7670_W_RCLK(0);
				//color = GPIOA->IDR&0xFF; //先低八位
				//color = (GPIOA->IDR&0xFF << 8);  //高八位
				Serial_SendByte(GPIOA->IDR&0xFF);//只有使用Serial_SendByte才可以被串口客户端识别
				OV7670_W_RCLK(1);
				
				OV7670_W_RCLK(0);
				//color = color | (GPIOA->IDR&0xFF << 8);//后高八位
				//color = color | (GPIOA->IDR&0xFF);
